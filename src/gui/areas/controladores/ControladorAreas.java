...
public class ControladorAreas implements IControladorAreas {
    ...
    public void ventanaGanaFoco(WindowEvent evt) {
        ...
        //  1. Se presiona el botón "Areas" en la ventana principal (se abre la ventana)
	//  2. Se vuelve de la ventana de alta de áreas (se sale de la VentanaAMArea, es decir, la ventana de alta de areas genera un evento windowevent)
        //  3. Se vuelve de borrar un área (la ventana AMArea genera un evento)
        ...
        JTable tablaAreas = this.ventana.verTablaAreas();
        if (tablaAreas.getModel() instanceof ModeloTablaAreas)   //2 y 3: se vuelve de la ventana AMArea, o de borrar un área (el if pregunta, el modelo de tablaAreas es un ModeloTablaArea?)
